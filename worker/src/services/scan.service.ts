...
export class ScanService {
  private async fetchRepoTree(owner: string, repo: string, branch: string, token: string) {
  ...
  private async fetchFileContent(owner: string, repo: string, path: string, token: string) {
  ...
  async processScanJob(data: any) {
    ...
    try {
      ...
      const treeData = await this.fetchRepoTree(owner, repo, branch, githubToken);
      ...
      for (const file of files) {
        try {
          const content = await this.fetchFileContent(owner, repo, file.path, githubToken);
          if (content) {
            codeContext += `\n--- File: ${file.path} ---\n${content}\n`;
          }
        } catch (err: any) {
